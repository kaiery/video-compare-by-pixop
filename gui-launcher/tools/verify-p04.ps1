# P04 fail-fast entry point. No old EXE checks after a failed build/test.
[CmdletBinding()]
param()
...
$steps=@(
    ...
    @{Script='verify-window.ps1';Arguments=@('-EvidenceName','p04-window')}
)
foreach($step in $steps) {
    ...
    if($LASTEXITCODE -ne 0){throw "P04 stopped at $($step.Script), exit $LASTEXITCODE. Later steps were not run."}
}
Write-Output 'PASS: P04 source/UI mappings, builds, parameter tests, GUI settings and input regressions.'
